...
//  CONFIG
// ─────────────────────────────────────────────
const API_TIMEOUT_MS = 25000;      // 25 s — safe for Vercel 30 s limit
const MAX_RETRIES    = 2;
const MAX_GEN_ATTEMPTS = 3;
const MIN_QUESTION_COUNT = 1;
const MAX_QUESTION_COUNT = 50;
const DEFAULT_QUESTION_COUNT = 10;
const MAX_INPUT_CHARS = 8000;      // guard against huge prompts
...
// ─────────────────────────────────────────────
//  1. PROMPT BUILDER
//     Returns a strict-JSON educational prompt.
// ─────────────────────────────────────────────
...
function buildQuizPrompt(sourceText, questionCount) {
    // Trim aggressively for token efficiency; keep ≤6000 chars
...
//     and objects accidentally returned instead of arrays.
// ─────────────────────────────────────────────
function safeParseJSON(raw) {
    ...
    // Attempt 2: extract first [...] or first {...}
    ...
    if (objMatch) {
        try {
            ...
            // If API accidentally returned a wrapper object with an array inside
...
function validateQuestion(q, idx) {
    ...
    if (typeof q.question !== 'string' || q.question.trim().length < 5)
        issues.push('Missing or too-short question string');
    if (!Array.isArray(q.options) || q.options.length !== 4)
        issues.push(`options must be array of 4; got ${Array.isArray(q.options) ? q.options.length : typeof q.options}`);
    if (!Number.isInteger(q.correctIndex) || q.correctIndex < 0 || q.correctIndex > 3)
        issues.push(`correctIndex out of range: ${q.correctIndex}`);
    return { valid: issues.length === 0, issues };
}

// ─────────────────────────────────────────────
//  4. AUTO-REPAIR
//     Tries to fix minor structural issues in a question.
//     Returns repaired question or null if unrepairable.
// ─────────────────────────────────────────────
function repairQuestion(q) {
    if (!q || typeof q !== 'object') return null;
    const r = { ...q };

    // Repair question string
    if (typeof r.question !== 'string') r.question = String(r.question || '').trim();
    if (r.question.length < 5) return null; // can't invent a question

    // Repair options array
    if (!Array.isArray(r.options)) {
        // Sometimes the model puts options as an object { "A": "...", "B": "...", ... }
        if (r.options && typeof r.options === 'object') {
            r.options = Object.values(r.options).slice(0, 4).map(String);
        } else {
            return null;
        }
    }
    ...
    // Ensure type + difficulty have valid values
...
// ─────────────────────────────────────────────
//  5. VALIDATE + REPAIR BATCH
//     Returns array of valid (possibly repaired) questions.
// ─────────────────────────────────────────────
function validateAndRepairBatch(rawArray) {
    if (!Array.isArray(rawArray)) return [];

    return rawArray.reduce((acc, q, idx) => {
        const { valid } = validateQuestion(q, idx);
        if (valid) {
            acc.push(q);
        } else {
            const repaired = repairQuestion(q);
            if (repaired) {
                const { valid: repairedValid } = validateQuestion(repaired, idx);
                if (repairedValid) acc.push(repaired);
                // silently drop if still invalid after repair
            }
        }
        return acc;
    }, []);
...
async function callChatAPI(promptText) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { ok: false, reply: null, errorType: isServerError ? 'server' : 'client', status: response.status };
        }
...
// ─────────────────────────────────────────────
//  7. FETCH WITH EXPONENTIAL BACKOFF
//     Retries only on server/network/timeout errors.
//     Skips retry on 4xx client errors (bad input).
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  8. FULL GENERATION PIPELINE
//     Ties everything together with exact-count validation.
// ─────────────────────────────────────────────
async function generateQuizQuestions(sourceText, questionCount, attemptNumber = 1) {
    ...
    if (!parsed) {
        // If parse failed completely, try once more with a fresh call
        return generateQuizQuestions(sourceText, safeQuestionCount, attemptNumber + 1);
    }
...
document.getElementById('generateBtn').addEventListener('click', async () => {
    ...
    // Show overlay, disable button to prevent duplicate submissions
...
function renderQuestion() {
    ...
    // Question type badge (only if type is present)
...
function selectAnswer(selectedIndex, btnElement) {
    ...
    nextBtn.textContent = currentQuestionIndex < activeQuizData.questions.length - 1
        ? 'Next Question ➔'
        : 'See Results ✨';
...
document.getElementById('questionCountInput').addEventListener('change', e => {
    updateQuestionCount(e.target.value);
});

document.getElementById('questionCountInput').addEventListener('blur', e => {
    updateQuestionCount(e.target.value);
});
