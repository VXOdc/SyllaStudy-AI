 ...
 * Primary:  Supabase `quiz` table
 * Fallback: localStorage (works fully offline)
 *
 * Table schema expected in Supabase:
 *   quiz (id uuid PK, data jsonb, user_id text, created_at timestamptz)
 *   `data` stores: { title, questions[], requestedQuestionCount, bestScore, createdAt }
 */
  ...
  try {
    ...
    if (legacy) {
      ...
      const normalized = Array.isArray(items)
        ? items.map((q) => ({ ...q, pendingSync: false }))
        : [];
